...
from .parallel_adapter import LlamaAdapterLayer
...
class ModelArgs:
    ...
    # hyper
    padapter_layers: str = '0-32'
    hyper_padapter_layers: str = '16-32'
    padapter_size: int = 16
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs, w_padapter=False, hyper_padapter=False):
        super().__init__()
        ...
        if self.w_padapter:
            self.p_adapter = LlamaAdapterLayer(self.dim, args.padapter_size, hyper_padapter)
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.padapter_layers_id = [x for span in params.padapter_layers.split(',') for x in range(int(span.split('-')[0]), int(span.split('-')[1]))]
        print(f'padapter_layers_id:{self.padapter_layers_id}')
        self.hyper_padapter_layers_id = [x for x in range(int(params.hyper_padapter_layers.split('-')[0]),int(params.hyper_padapter_layers.split('-')[1]))]
        print(f'hyper_padapter_layers_id:{self.hyper_padapter_layers_id}')

        self.layers = torch.nn.ModuleList()
        for layer_id in range(params.n_layers):
            w_padapter = False
            hyper_padapter = False
            if layer_id in self.padapter_layers_id:
                w_padapter = True
                if layer_id in self.hyper_padapter_layers_id:
                    hyper_padapter = True 
            self.layers.append(TransformerBlock(layer_id, 
                                                params,
                                                w_padapter=w_padapter, 
                                                hyper_padapter=hyper_padapter))
