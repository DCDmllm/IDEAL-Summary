...
from .parallel_adapter import LlamaParameterGenerator
...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, args, llama_ckpt_dir, llama_tokenizer):
        ...
        model_args: ModelArgs = ModelArgs(
            ...
            w_bias = args.w_bias,
            padapter_layers = args.padapter_layers,
            hyper_padapter_layers = args.hyper_padapter_layers,
            padapter_size = args.padapter_size,
        ...
        # hypernetwork
        # parallel adapter
        self.hyper_padapter_layers_id = self.llama.hyper_padapter_layers_id
        self.hyper_padapter_start = None
        if self.hyper_padapter_layers_id:
            self.hyper_padapter_start = self.hyper_padapter_layers_id[0]
            ...
            compress_dim = 64
            self.padapter_hyper_net = LlamaParameterGenerator(len(self.hyper_padapter_layers_id), embed_size, compress_dim, model_args.dim, model_args.padapter_size, common_encoder=model_args.common_encoder, serial_generate=model_args.serial_generate)
            self.padapter_hyper_net.cuda()
    ...
    def forward(self, tokens, labels, prompt_mask):
        ...
        for layer in self.llama.layers[:self.hyper_padapter_start]:
            h = layer(h, 0, freqs_cis, mask)
        
        if self.hyper_padapter_start:
            ...
            if not self.serial_generate: # parallel generate params
                ...
                params = self.padapter_hyper_net(pooling_states.detach()) # 不回传梯度效果略好

                for hi,layer in enumerate(self.llama.layers[self.hyper_padapter_start:]):
                    param = params[hi]
                    adapter_layer = layer.p_adapter
                    adapter_layer.clear_adapter()
                    adapter_layer.apply_adapter_params(param[0], param[1], param[2], param[3])

                for layer in self.llama.layers[self.hyper_padapter_start:]:
                    h = layer(h, 0, freqs_cis, mask)

            else:  # serial generate params
                for i,layer in enumerate(self.llama.layers[self.hyper_padapter_start:]):
                    pooling_states = torch.sum(h * prompt_mask.unsqueeze(-1), dim=1) / denom
                    if self.hyper_input_type == 'both':
                        pooling_states1 = torch.sum(h * prompt_mask1.unsqueeze(-1), dim=1) / denom1
                        pooling_states = torch.cat((pooling_states, pooling_states1), -1)
                    param = self.padapter_hyper_net(pooling_states.detach(), hyper_index=i)
