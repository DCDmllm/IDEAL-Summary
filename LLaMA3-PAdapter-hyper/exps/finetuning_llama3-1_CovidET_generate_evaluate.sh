...
epochs=1
dataset="CovidET"
max_seq_len=1000 # 1000 on 3090, 8000 need 80G GPU
min_gen_len=30
max_gen_len=128
...
padapter_size=32
padapter_layers="0-32"
hyper_padapter_layers="16-32"

path="/home"
llama="Meta-Llama-3.1-8B-Instruct"
output_dir="${path}/outputs/LLaMA3-1-PAdapter-hyper/${dataset}/llama${llama}_b32_epoch${epochs}_warme1_padaptersize${padapter_size}_hyperlayers${hyper_padapter_layers}_diffe_parallel_blr6e3_lossOlabels${loss_only_labels}_maxseq${max_seq_len}_flashattn${flash_attention2}_bf16${bf16}_${tag}/"
...
torchrun --nproc_per_node $num_devices --master_port=$master_port main_finetune.py \
    ...
    --padapter_size $padapter_size \
    --padapter_layers $padapter_layers \
    --hyper_padapter_layers $hyper_padapter_layers \
