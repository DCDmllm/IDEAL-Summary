...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, args, llama_ckpt_dir, llama_tokenizer):
        ...
        model_args: ModelArgs = ModelArgs(
            ...
            hyper_prompt_layers = args.hyper_prompt_layers,
        ...
        # hypernetwork
        # prompt
        self.hyper_prompt_layers_id = self.llama.hyper_prompt_layers_id
        self.hyper_prompt_start = None
        if self.hyper_prompt_layers_id:
            self.hyper_prompt_start = self.hyper_prompt_layers_id[0]
            ...
            compress_dim = 64
            self.prompt_hyper_net = PromptGenerator(len(self.hyper_prompt_layers_id), embed_size, compress_dim, model_args.dim, model_args.prompt_len, common_encoder=model_args.common_encoder, serial_generate=model_args.serial_generate)
            self.prompt_hyper_net.cuda()
    ...
    def forward(self, tokens, labels, prompt_mask):
        ...
        for layer in self.llama.layers[:self.hyper_prompt_start]:
            h = layer(h, 0, freqs_cis, mask)
            
        if self.hyper_prompt_start:
            ...
            if not self.serial_generate: # parallel generate params
                ...
                prompt_params = self.prompt_hyper_net(pooling_states.detach()) # 不回传梯度效果略好

                for hi,layer in enumerate(self.llama.layers[self.hyper_prompt_start:]):
                    layer.attention.apply_param(prompt_params[hi])
                    h = layer(h, 0, freqs_cis, mask)

            else:  # serial generate params
                for i,layer in enumerate(self.llama.layers[self.hyper_prompt_start:]):
                    pooling_states = torch.sum(h * prompt_mask.unsqueeze(-1), dim=1) / denom
                    if self.hyper_input_type == 'both':
                        pooling_states1 = torch.sum(h * prompt_mask1.unsqueeze(-1), dim=1) / denom1
                        pooling_states = torch.cat((pooling_states, pooling_states1), -1)
                    prompt_param = self.prompt_hyper_net(pooling_states.detach(), hyper_index=i)
                    layer.attention.apply_param(prompt_param)
                    h = layer(h, 0, freqs_cis, mask)
